...
class QuestionSetControllerCrudTest {
    ...
    @Test
    @DisplayName("GET /api/questionsets/public - 공개 질문세트 목록 조회 성공")
    void getPublicQuestionSets_Success() throws Exception {
        // Given
        QuestionSetListResponse listResponse = QuestionSetListResponse.builder()
                .questionSetId(2L)
                .title("공개 질문세트")
                .description("모두가 사용할 수 있는 질문세트")
                .versionNumber(1)
                .parentVersionId(null)
                .isPublic(true)
                .ownerNickname("다른사용자")
                .questionCount(5)
                .tags(Arrays.asList(createTagResponse("Java")))
                .createdAt(LocalDateTime.now())
                .lastModifiedAt(LocalDateTime.now())
                .build();

        Page<QuestionSetListResponse> pageResponse = new PageImpl<>(Arrays.asList(listResponse));
        given(questionSetService.getSharedQuestionSetsNew(TEST_USER_ID, 0, 10)).willReturn(pageResponse);

        setAuthentication();

        // When & Then
        mockMvc.perform(get("/api/questionsets/public")
                        .param("page", "0")
                        .param("size", "10"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.message").value("공개 질문세트 목록 조회가 완료되었습니다."))
                .andExpect(jsonPath("$.data.content").isArray())
                .andExpect(jsonPath("$.data.content[0].isPublic").value(true));
